...
grb::RC LazyEvaluation::addFunctorStage(
	...
	if( opcode == Opcode::BLAS2_VXM_GENERIC ) {
		...
		// search for pipelines with shared data
		for( auto pt = pipelines.begin(); pt != pipelines.end(); pt++ ) {
	...
	if( shared_data_pipelines.empty() ) {
		...
		if( empty_pipeline != nullptr ) {
			( *empty_pipeline).addFunctorStage( std::move( functorStage ), opcode );
			...
			pipeline.addFunctorStage( std::move( functorStage ), opcode );
		...
		auto ptr = ( *(shared_data_pipelines.begin()) );

		// the stage is added in the current pipeline which may be empty if it
		// overwrites the input of SpMV
		// it is not necessary to deallocate/release this pipeline
		( *ptr ).addFunctorStage( std::move( functorStage ), opcode );
		...
		// all pipelines with which the current pipelines shares data will be merged
		// under the first pipeline
		auto union_pipeline = ( *(shared_data_pipelines.begin()) );

		for( auto st = ++shared_data_pipelines.begin(); st != shared_data_pipelines.end(); st++ ) {
			( *union_pipeline ).merge( *( *st ) );
		}

		// the stage is added in the merged pipeline
		// it is not necessary to deallocate/release this pipeline
		( *union_pipeline ).addFunctorStage( std::move( functorStage ), opcode );
