...
void grb_program( const Input & in, grb::RC & rc ) {
	...
	rc = grb::mxv< grb::descriptors::safe_overlap >(
		vector, even_mask, matrix, vector, odd_mask, grb::Semiring< grb::operators::add< double >, grb::operators::mul< double >, grb::identities::zero, grb::identities::one >() );
	if( rc != SUCCESS ) {
...
#endif
		if( ! grb::utils::equals( entry.second, one[ entry.first ], static_cast< double >( maxAccumOne ) ) ) {
			std::cerr << "\t entry ( " << entry.first << ", " << entry.second << " ) does not equal expected value " << one[ entry.first ] << " in step one\n";
			rc = FAILED;
		}
	}
	if( rc == FAILED ) {
	...
	rc = grb::mxv< grb::descriptors::safe_overlap >(
		vector, odd_mask, matrix, vector, even_mask, grb::Semiring< grb::operators::add< double >, grb::operators::mul< double >, grb::identities::zero, grb::identities::one >() );
	if( rc != SUCCESS ) {
	...
	for( const auto & entry : vector ) {
		...
		std::cout << "( " << entry.first << ", " << entry.second << " )\n";
#endif
		if( ! grb::utils::equals( entry.second, two[ entry.first ], static_cast< double >( maxAccumTwo ) ) ) {
			std::cerr << "\t entry ( " << entry.first << ", " << entry.second << " ) does not equal expected value " << two[ entry.first ] << " in step 2\n";
			rc = FAILED;
		}
	}
	if( rc == FAILED ) {
	...
	rc = grb::vxm< grb::descriptors::safe_overlap >(
		vector, odd_mask, vector, even_mask, matrix, grb::Semiring< grb::operators::add< double >, grb::operators::mul< double >, grb::identities::zero, grb::identities::one >() );
	if( rc != SUCCESS ) {
	...
	for( const auto & entry : vector ) {
		...
		std::cout << "( " << entry.first << ", " << entry.second << " )\n";
#endif
		if( ! grb::utils::equals( entry.second, three[ entry.first ], static_cast< double >( maxAccumThree ) ) ) {
			std::cerr << "\t entry ( " << entry.first << ", " << entry.second << " ) does not equal expected value " << three[ entry.first ] << " in step 3\n";
			rc = FAILED;
		}
	}
	if( rc == FAILED ) {
...
#endif
	rc = grb::vxm< grb::descriptors::safe_overlap >(
		vector, even_mask, vector, odd_mask, matrix, grb::Semiring< grb::operators::add< double >, grb::operators::mul< double >, grb::identities::zero, grb::identities::one >() );
	if( rc != SUCCESS ) {
	...
	for( const auto & entry : vector ) {
		...
		std::cout << "( " << entry.first << ", " << entry.second << " )\n";
#endif
		if( ! grb::utils::equals( entry.second, four[ entry.first ], static_cast< double >( maxAccumFour ) ) ) {
			std::cerr << "\t entry ( " << entry.first << ", " << entry.second << " ) does not equal expected value " << four[ entry.first ] << " in step 4\n";
			rc = FAILED;
		}
	}
	if( rc == FAILED ) {
