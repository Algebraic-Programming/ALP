...
#if ! defined _H_GRB_BANSHEE_COORDINATES
...
namespace grb {
	...
	namespace internal {
		 ...
		 * coordinates. Its use is internal via, e.g., grb::Vector< T, banshee >.
		...
		public:
			...
			typedef typename grb::config::VectorIndexType StackType;

			/** Local update type for use with #asyncAssign and #joinAssign. */
			typedef std::vector< StackType, banshee_allocator< StackType > > Update;
		...
		private:
			...
			/**
			 * A buffer of exactly \f$ P \f$ elements, where \f$ P \f$ is the number
		...
		public:
			 ...
			 * Empty update.
			 */
			static inline Update EMPTY_UPDATE() {
				Update ret;
				ret.reserve( 1 );
				return ret;
			}
			...
			static inline size_t bufferSize( const size_t dim ) noexcept {
				...
				int P = 1;
				ret += ( P + 1 ) * sizeof( StackType );
				return ret;
			}

			/** Base constructor. Creates an empty coordinates list of dimension 0. */
			inline Coordinates() noexcept : _assigned( NULL ), _stack( NULL ), _buffer( NULL ), _n( 0 ), _cap( 0 ) {}

			/**
			 * Move constructor.
			 * @param[in] x The Coordinates instance to move contents from.
			 */
			inline Coordinates( Coordinates && x ) noexcept : _assigned( x._assigned ), _stack( x._stack ), _buffer( x._buffer ), _n( x._n ), _cap( x._cap ) {
				x._assigned = NULL;
				x._stack = NULL;
				x._buffer = NULL;
				x._n = x._cap = 0;
			...
			inline Coordinates( const Coordinates & x ) noexcept : _assigned( x._assigned ), _stack( x._stack ), _buffer( x._buffer ), _n( x._n ), _cap( x._cap ) {
			...
			inline ~Coordinates() noexcept {
				// done (the #_assigned and #_stack memory blocks are not managed by this class)
			}
			...
			void rebuild( const bool dense ) noexcept {
				...
				if( dense ) {
#ifdef _DEBUG
					printf( "rebuildSparsity: dense case\n" );
#endif
					assignAll();
					return;
				}

				StackType * __restrict__ counts = NULL;

				{
#ifdef _DEBUG
					printf( "rebuildSparsity: sparse case\n" );
#endif
					...
					const size_t s = 0;
					counts = _buffer;
					const size_t bs = _cap / P + ( ( _cap % P == 0 ) ? 0 : 1 );
					const size_t start = s * bs;
					const size_t end = start + bs > _cap ? _cap : start + bs;
#ifdef _DEBUG
					printf( "Coordinates::rebuild: thread %d has range %d -- "
							"%d\n",
						s, start, end );
#endif
...
#ifdef _DEBUG
					printf( "Coordinates::rebuild: thread %d found %d "
							"nonzeroes\n",
						(int)s, (int)local_count );
#endif

#ifdef _DEBUG
					printf( "Coordinates::rebuild: thread 0 found %d "
							"nonzeroes.\n",
						counts[ 0 ] );
#endif
			...
			void rebuildGlobalSparsity( const Coordinates & localSparsity, const size_t offset ) noexcept {
				...
				if( isDense() ) {
					...
					while( k < end ) {
						...
						if( _assigned[ i ] ) {
							// this nonzero is only valid if it is in the range of the localCoordinates
			 ...
			 */
			inline bool asyncAssign( const size_t i, Update & localCount ) noexcept {
			 ...
			 * updates recorded via #asyncAssign are deleted)-- i.e., the instance
			 * will be \a empty.
			 *
			 ...
			 */
			inline void joinUpdate( Update & update ) noexcept {
				// catch trivial case
				if( update.empty() ) {
					return;
				}
				...
				const size_t newSize = _n + update.size();
				assert( _n <= _cap );
				assert( newSize <= _cap );
				for( size_t i = 0; i < update.size(); ++i ) {
					_stack[ _n + i ] = update[ i ];
				}
				// update number of nonzeroes
				_n = newSize;
				update.clear();
			}
...
#endif // end `_H_GRB_BANSHEE_COORDINATES'
