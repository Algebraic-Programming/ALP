...
#if ! defined _H_GRB_BANSHEE_VECTOR
#define _H_GRB_BANSHEE_VECTOR
...
// Empty function needed because of the lack of stdlib support on banshee
namespace std {
...
namespace grb {
	...
	namespace internal {
		...
		inline C & getCoordinates( Vector< D, banshee, C > & x ) noexcept;

		/** TODO documentation */
		template< typename D, typename C >
		inline const C & getCoordinates( const Vector< D, banshee, C > & x ) noexcept;

		/** TODO documentation */
		template< typename D, typename C >
		inline D * getRaw( Vector< D, banshee, C > & x ) noexcept;

		/** TODO documentation */
		template< typename D, typename C >
		inline const D * getRaw( const Vector< D, banshee, C > & x ) noexcept;

		template< typename D >
		inline internal::Compressed_Storage< D, grb::config::RowIndexType, grb::config::NonzeroIndexType > & getCRS( Matrix< D, banshee > & A ) noexcept;

		template< typename D >
		inline const internal::Compressed_Storage< D, grb::config::RowIndexType, grb::config::NonzeroIndexType > & getCRS( const Matrix< D, banshee > & A ) noexcept;

		template< typename D >
		inline internal::Compressed_Storage< D, grb::config::ColIndexType, grb::config::NonzeroIndexType > & getCCS( Matrix< D, banshee > & A ) noexcept;

		template< typename D >
		inline const internal::Compressed_Storage< D, grb::config::ColIndexType, grb::config::NonzeroIndexType > & getCCS( const Matrix< D, banshee > & A ) noexcept;
	...
	class Vector< D, banshee, MyCoordinates > {
	...
	private:
		...
		void initialize( D * const raw_in, void * const assigned_in, bool assigned_initialized, void * const buffer_in, const size_t cap_in ) {
			...
			if( raw_in != NULL || assigned_in != NULL || buffer_in != NULL ) {
				...
				// grb::Vector::initialize (banshee or banshee_omp).
			...
			const RC rc = grb::utils::alloc( _raw, cap_in, true,
				_raw_deleter, // values array
			...
			// catch errors
			if( rc == OUTOFMEM ) {
				//					throw std::runtime_error( "Out-of-memory during banshee Vector memory allocation" );
			} else if( rc != SUCCESS ) {
	...
	public:
		 ...
		 * a lambda banshee to be of the same type as a regular C++ banshee. The
		 * restrictions as specified in Vector::lambda_banshee, however, still
		 * apply.
		 *
		 * @see Vector::lambda_banshee for the user-level specification.
		 */
		typedef D & lambda_banshee;

		/**
		 * A standard iterator for the Vector< D, banshee > class.
		...
		private:
			 ...
			 *
			 * \internal Can only be called from the grb::Vector< D, banshee > class.
			 *
...
#ifdef _DEBUG
				printf( "\t ConstIterator at process %d / %d translated index "
						"% to %d\n",
					s, P, index, global_index );
#endif
		...
		public:
			/** Default constructor. */
			ConstIterator() noexcept : container( NULL ), position( 0 ), max( 0 ), s( grb::spmd<>::pid() ), P( grb::spmd<>::nprocs() ) {}
		...
		typedef ConstIterator< internal::Distribution< banshee > > const_iterator;
		 ...
		 * @see Vector for the user-level specfication.
		 */
		template< class ActiveDistribution = internal::Distribution< banshee > >
		 ...
		 *
		 * \warning This function is only defined for the banshee backend--
		 *          thus switching backends may cause your code to not compile.
		 *
		 * @return A const banshee to the raw data this vector contains.
		 *
		...
		/**
		 * Returns a #lambda_banshee to the i-th element of this vector. This
		 * banshee may be modified.
		 *
		 ...
		 */
		lambda_banshee operator[]( const size_t i ) {
			// sanity checks
		...
		/**
		 * Returns a #lambda_banshee to the i-th element of this vector. This
		 * banshee may \em not be modified.
		 *
		 * This implementation asserts only valid elements are requested.
		 *
		 * \note Compile with the \a NDEBUG flag set to disable checks for this
		 ...
		 * @see Vector::operator[] for the user-level specification.
		 */
		lambda_banshee operator[]( const size_t i ) const {
			// sanity checks
			assert( i < _coordinates.size() );
			assert( _coordinates.assigned( i ) );
			// directly return the banshee
			return _raw[ i ];
		}
		...
		/*template< class Monoid >
		    lambda_banshee operator()( const size_t i, const Monoid &monoid = Monoid() ) {
		        //sanity checks
		        assert( i < _cap );
		        ...
		        //and finally return the banshee
		        return _raw[ i ];
		    }*/
	};
...
#endif // end ``_H_GRB_BANSHEE_VECTOR''
