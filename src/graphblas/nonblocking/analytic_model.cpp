...
AnalyticModel::AnalyticModel( const size_t data_type_size, const size_t vector_size, const size_t accessed_vectors )
	...
	const bool manual_choice = grb::config::IMPLEMENTATION< nonblocking >::isManualTileSize();
	if ( !manual_choice ) {
		...
		constexpr size_t min_tile_size = grb::config::IMPLEMENTATION< nonblocking >::analyticModelMinimumTileSize();
		constexpr double l1_cache_usage_percentage = grb::config::IMPLEMENTATION< nonblocking >::analyticModelL1CacheUsagePercentage();

		// A tile size estimation based on the data that fit in L1 cache.
		const size_t cache_based_tile_size = ( l1_cache_size / size_of_data_type ) / ( num_accessed_vectors );
		...
		tile_size_estimation = ( cache_based_tile_size < cores_based_tile_size ) ? cache_based_tile_size : cores_based_tile_size;
		// It assumes that the L1 cache may contain other data and takes the size of these data into account.
		tile_size_estimation *= l1_cache_usage_percentage;
		// It ensures that the tile size is sufficiently large to successfully amortize the runtime overhead.
		tile_size_estimation = ( min_tile_size > tile_size_estimation ) ? min_tile_size : tile_size_estimation;
	} else {
		// It determines the tile size and the number of threads manually.
		// A fixed tile size and number of threads is used for the execution of all pipelines.
		tile_size_estimation = grb::config::IMPLEMENTATION< nonblocking >::manualFixedTileSize();
	}
