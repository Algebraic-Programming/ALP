	...
	for( size_t p = 0; p < lpf_data.P; ++p ) {
		...
		if( p == lpf_data.s ) {
			if( local_ss.str().empty() ) {
				std::cerr << "Process " << lpf_data.s << ":  [nothing to print]\n";
			} else {
...
void grb_program( const size_t &n, RC &rc ) {
	...
	// coordinates match, since it should always be: i == j
	const Matrix< D, reference > &local_matrix =
		internal::getLocal( I_distributed );
	...
	std::stringstream local_ss;
	{
		...
		for(size_t i = 0; i < nrows( local_matrix ); ++i ) {
			for( size_t k = crs.col_start[i]; k < crs.col_start[ i + 1 ]; ++k ) {
				...
				const size_t col_pid = internal::Distribution<>::offset_to_pid( j,
					ncols(I_distributed), lpf_data.P );
				const size_t col_off = internal::Distribution<>::local_offset(
					ncols(I_distributed), col_pid, lpf_data.P );
				const auto global_i = internal::Distribution<>::local_index_to_global( i,
					nrows(I_distributed), lpf_data.s, lpf_data.P );
				const auto global_j = internal::Distribution<>::local_index_to_global(
					j - col_off, ncols(I_distributed), col_pid, lpf_data.P );

				if (global_i != global_j) {
					local_ss << "  Wrong coordinate in CRS found at: ( "
					    << std::setw(3) << i << ", " << std::setw(3) << global_j << " )  "
					    << "--(mapped to global)-->  ( "
