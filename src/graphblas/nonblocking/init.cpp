...
size_t grb::config::ANALYTIC_MODEL::manual_fixed_tile_size = grb::config::IMPLEMENTATION< nonblocking >::analyticModelMinimumTileSize();
size_t grb::config::ANALYTIC_MODEL::num_threads = grb::config::OMP::threads();

template<>
grb::RC grb::init< grb::nonblocking >( const size_t s, const size_t P, void * const data ) {
	...
	// made at run-time by the analytic model and may differ for different pipelines.
	const char *t = getenv( "GRB_NONBLOCKING_TILE_SIZE" );
	if( t != NULL ) {
		config::ANALYTIC_MODEL::manual_tile_size = true;
		config::ANALYTIC_MODEL::manual_fixed_tile_size = atoi( t );
	} else {
		config::ANALYTIC_MODEL::manual_tile_size = false;
	}

	std::cerr << "Info: grb::init (nonblocking) called. OpenMP is set to utilise "
				<< config::ANALYTIC_MODEL::num_threads << " threads and the tile size for nonblocking execution is chosen "
				<< ( config::ANALYTIC_MODEL::manual_tile_size ? "manually" : "automatically.\n" );
	if( config::ANALYTIC_MODEL::manual_tile_size ) {
		std::cerr << " and is equal to " << config::ANALYTIC_MODEL::manual_fixed_tile_size << "." << std::endl;
	}
