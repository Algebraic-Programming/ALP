...
#define USE1 (void)printf( "Usage: %s <Host server> <PID> <#processes> <port>\n", argv[ 0 ] );
#define USE2                                                                   \
	(void)printf( "  <host server> This argument can be a string (e.g., "      \
...
#define USE3                                                                  \
	(void)printf( "     <PID>      The integer value for <PID> must be in "   \
...
#define USE5                                                                   \
	(void)printf( "     <port>     This argument must either be a service "    \
...
int main( int argc, char ** argv ) {
	...
	if( argc != 5 ) {
		USE1 USE2 USE3 USE4 USE5 return 0;
	}
	...
	if( host == NULL || host[ 0 ] == '\0' ) {
		(void)fprintf( stderr, "Invalid hostname (%s).\n", argv[ 1 ] );
		USE2 return 100;
	}
	...
	if( ! grb::utils::is_in_normalized_range( s, P ) ) {
		(void)fprintf( stderr, "Invalid value for PID (%s, parsed as %zd).\n", argv[ 2 ], (size_t)s );
		USE4 return 300;
	}
	if( port == NULL || port[ 0 ] == '\0' ) {
		(void)fprintf( stderr, "Invalid value for port name or number (%s).\n", argv[ 4 ] );
		USE5 return 400;
	}

	// initialise MPI
	if( MPI_Init( NULL, NULL ) != MPI_SUCCESS ) {
		(void)fprintf( stderr, "MPI_Init returns with non-SUCCESS exit code.\n" );
		return 10;
	}
	...
	if( initrc != LPF_SUCCESS ) {
		(void)printf( "Error in call to lpf_mpi_initialize_over_tcp.\n" );
		return 500;
	}
	...
	// print debug message
	if( initrc == LPF_SUCCESS ) {
		(void)printf( "Initialisation complete, calling hook...\n" );
	} else {
		(void)printf( "Initialization failed...\n" );
	}

	// call SPMD
	const lpf_err_t spmdrc = lpf_hook( init, &spmd, args );
	if( spmdrc != LPF_SUCCESS ) {
		(void)printf( "Error in call to lpf_hook.\n" );
		return 600;
	}
	...
	if( finrc != LPF_SUCCESS ) {
		(void)printf( "Error in call to lpf_mpi_finalize.\n" );
		return 700;
	}

	// finalise MPI
	if( MPI_Finalize() != MPI_SUCCESS ) {
		(void)fprintf( stderr, "MPI_Finalize() returns with non-SUCCESS exit code.\n" );
		return 20;
	}
