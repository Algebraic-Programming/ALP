...
namespace grb {
	...
	namespace internal {
		...
		public:
			...
			inline Coordinates() noexcept : _assigned( NULL ), _stack( NULL ), _buffer( NULL ), _n( 0 ), _cap( 0 ), _buf( 0 ) {}

			/**
			 * Move constructor.
			 * @param[in] x The Coordinates instance to move contents from.
			 */
			inline Coordinates( Coordinates && x ) noexcept : _assigned( x._assigned ), _stack( x._stack ), _buffer( x._buffer ), _n( x._n ), _cap( x._cap ), _buf( x._buf ) {
				x._assigned = NULL;
				x._stack = NULL;
				x._buffer = NULL;
			...
			inline Coordinates( const Coordinates & x ) noexcept : _assigned( x._assigned ), _stack( x._stack ), _buffer( x._buffer ), _n( x._n ), _cap( x._cap ), _buf( x._buf ) {
...
#endif
				{
...
#endif
#ifdef _DEBUG
					std::cout << "Coordinates::rebuild: thread " << s << " has range " << start << "--" << end << "\n";
#endif
					...
					for( size_t i = start; i < end; ++i ) {
						if( _assigned[ i ] ) {
							...
							//								#pragma omp critical
							// #endif
							//								std::cout << "\tnonzero found at local index " << i << "\n";
							//#endif
							(void)++local_count;
						}
					}
					...
					#pragma omp critical
#endif
					std::cout << "Coordinates::rebuild: thread " << s << " found " << local_count << " nonzeroes." << std::endl;
#endif
...
#ifdef _DEBUG
						std::cout << "Coordinates::rebuild: thread 0 found " << counts[ 0 ] << " nonzeroes.\n";
#endif
						for( size_t k = 1; k < P; ++k ) {
#ifdef _DEBUG
							std::cout << "Coordinates::rebuild: thread " << k << " found " << counts[ k ] << " nonzeroes.\n";
#endif
							counts[ k ] += counts[ k - 1 ];
						}
		...
		dynamic, config::CACHE_LINE_SIZE::value() )
			...
			inline bool joinUpdate( Update & update ) noexcept {
...
#ifdef _DEBUG
				#pragma omp critical
				std::cout << t << ": joinUpdate called. Has " << _n << " nonzeroes on entry. Thread-local stack has " << update[ 0 ] << " nonzeroes.\n";
#endif
				StackType * __restrict__ const pfBuf = _buffer + config::IMPLEMENTATION< reference >::vectorBufferSize( _cap, T );
				...
				const size_t global_start = t * global_bs > pfBuf[ T ] ? pfBuf[ T ] : t * global_bs;
				const size_t global_end = global_start + global_bs > pfBuf[ T ] ? pfBuf[ T ] : global_start + global_bs;
				const size_t global_length = global_end - global_start;
#ifdef _DEBUG
				#pragma omp critical
				std::cout << "\t Thread " << t << " has range " << global_start << " -- " << global_end << "\n";
#endif
				if( global_length > 0 ) {
					...
					for( size_t t_cur = t_start; t_cur < t_end; ++t_cur ) {
						/*#ifdef _DEBUG
						    #pragma omp critical
						    std::cout << "\t Thread " << t << " processing nonzero " << global_count << " / " << global_length << " using the local stack of thread " << t_cur << " starting from local
 index " << local_cur << ".\n"; #endif*///DBG
						assert( local_cur <= pfBuf[ t_cur + 1 ] - pfBuf[ t_cur ] );
						StackType * __restrict__ const cur_stack = _buffer + t_cur * stack_offset + 1;
						for( ; global_count < global_length && local_cur < pfBuf[ t_cur + 1 ] - pfBuf[ t_cur ]; ++global_count, ++local_cur ) {
...
#ifdef _DEBUG
				#pragma omp critical
				std::cout << "\t Thread " << t << " exiting joinUpdate. New nonzero count is " << _n << "\n";
#endif
