...
{
	// 32 elements should be sufficient to avoid dynamic memory allocation for the pipelines built at run-time
	pipelines.reserve( 1 << 6 );
	shared_data_pipelines.reserve( 1 << 6 );
}

grb::RC LazyEvaluation::addStage( const Pipeline::stage_type &&func, Opcode opcode, const size_t n, const size_t data_type_size, const bool dense_descr, const bool dense_mask,
								...
								const void * const input_matrix )
{
	...
	if( opcode == Opcode::BLAS2_VXM_GENERIC ) {
		...
		// TODO: the matrix is not currently added as input and thus not taken into account for data dependence analysis
		...
		for( std::vector< Pipeline >::iterator pt = pipelines.begin(); pt != pipelines.end(); pt++ ) {
			...
			// it's still possibe that the pipeline has to be executed to avoid overwriting the input vectors of SpMV
			if( !pipeline_executed ) {

				// first we check for shared data with the write-access vectors for efficiency
				// and only later we check for read-only vectors that don't enforce pipeline execution
		...
		for( std::vector< Pipeline >::iterator pt = pipelines.begin(); pt != pipelines.end(); pt++ ) {
			...
			if( !pipeline_executed ) {
				...
				if( opcode == Opcode::BLAS1_UNZIP ) {
					...
					// it's still necessary to execute the pipeline if the second output of unzip
					// overwrites any of the the input vectors of SpMV
	...
	if( !( opcode == Opcode::IO_SET_SCALAR || opcode == Opcode::IO_SET_MASKED_SCALAR || opcode == Opcode::IO_SET_VECTOR || opcode == Opcode::IO_SET_MASKED_VECTOR ||
			...
			opcode == Opcode::BLAS1_FOLD_VECTOR_VECTOR_GENERIC || opcode == Opcode::BLAS1_FOLD_MASKED_VECTOR_VECTOR_GENERIC ||
			opcode == Opcode::BLAS1_EWISEAPPLY || opcode == Opcode::BLAS1_MASKED_EWISEAPPLY || opcode == Opcode::BLAS1_EWISEMULADD_DISPATCH || opcode == Opcode::BLAS1_DOT_GENERIC ||
			opcode == Opcode::BLAS1_EWISELAMBDA || opcode == Opcode::BLAS1_EWISEMAP || opcode == Opcode::BLAS1_ZIP || opcode == Opcode::BLAS1_UNZIP ||
			opcode == Opcode::BLAS2_VXM_GENERIC ) ) {
		std::cerr << "error:Data Dependence Analysis has not been implemented for the operation with code " << static_cast< unsigned int >( opcode ) << std::endl;
		exit( 1 );
	}
	...
	for( std::vector< std::vector< Pipeline >::iterator >::iterator st = shared_data_pipelines.begin(); st != shared_data_pipelines.end(); st++ ) {
		if( (*(*st)).getContainersSize() != n ) {
			std::cerr << "error:Data Dependence Analysis detected data-dependent operations on vectors of different size" << std::endl;
			exit( 1 );
		}
	}
#endif
	...
	// and these data dependences do not allow this stage to be inserted into such a pipeline
	// we know that we can now merge all the remaining pipelines with which the current stage
	// shares data an then add the current stage at the end of the new pipeline
	// for efficiency, we consider the three following cases
	if( shared_data_pipelines.empty() ) {
		// if none of the current pipelines shares any data, the stage is added in a new pipeline
		...
		if( empty_pipeline != nullptr ) {
			( *empty_pipeline).addStage( std::move( func ), opcode, n, data_type_size, dense_descr, dense_mask,
			...
			pipeline.addStage( std::move( func ), opcode, n, data_type_size, dense_descr, dense_mask,
								output_vector_ptr, output_aux_vector_ptr,
								coor_output_ptr, coor_output_aux_ptr,
								input_a_ptr, input_b_ptr, input_c_ptr, input_d_ptr,
								coor_a_ptr, coor_b_ptr, coor_c_ptr, coor_d_ptr,
		...
		// the stage is added in the current pipeline which may be empty if it overwrites the input of SpMV
		// it is not necessary to deallocate/release this pipeline
		( *ptr ).addStage( std::move( func ), opcode, n, data_type_size, dense_descr, dense_mask,
							output_vector_ptr, output_aux_vector_ptr,
							coor_output_ptr, coor_output_aux_ptr,
							input_a_ptr, input_b_ptr, input_c_ptr, input_d_ptr,
							coor_a_ptr, coor_b_ptr, coor_c_ptr, coor_d_ptr,
		...
		// all pipelines with which the current pipelines shares data will be merged under the first pipeline
		...
		// it is not necessary to deallocate/release this pipeline
		( *union_pipeline ).addStage( std::move( func ), opcode, n, data_type_size, dense_descr, dense_mask,
										output_vector_ptr, output_aux_vector_ptr,
										coor_output_ptr, coor_output_aux_ptr,
										input_a_ptr, input_b_ptr, input_c_ptr, input_d_ptr,
										coor_a_ptr, coor_b_ptr, coor_c_ptr, coor_d_ptr,
...
grb::RC LazyEvaluation::addeWiseLambdaStage( const Pipeline::stage_type &&func, Opcode opcode, const size_t n, const size_t data_type_size, const bool dense_descr,
...
grb::RC LazyEvaluation::execution( const void * const container )
{
	...
	for( std::vector< Pipeline >::iterator pt = pipelines.begin(); pt != pipelines.end(); pt++ ) {
		...
		// a single pipeline is executed, and in the case of returning an error, it is handled correctly
...
grb::RC LazyEvaluation::execution()
{
	...
	for( std::vector< Pipeline >::iterator pt = pipelines.begin(); pt != pipelines.end(); pt++ ) {
		...
		//TODO: if a pipeline returns an error code, the rest of the pipelines are not executed, is that okay?
		rc = (*pt).execution();
		if( rc != SUCCESS ) {
			return rc;
		}
