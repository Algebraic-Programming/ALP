...
#ifndef _H_GRB_TUTORIAL_IO
#define _H_GRB_TUTORIAL_IO
...
namespace grb {
	...
	/**
	 * \defgroup IO Data Ingestion -- tutorial backend
	 * @{
	 */
	...
	uintptr_t getID( const Matrix< InputType, tutorial, RIT, CIT, NIT > &A ) {
		assert( nrows(A) > 0 );
		assert( ncols(A) > 0 );
		return A.id;
	}
	...
	size_t size( const Vector< DataType, tutorial, Coords > &x ) noexcept {
	...
	template< typename InputType, typename RIT, typename CIT, typename NIT >
	size_t nrows(
		const Matrix< InputType, tutorial, RIT, CIT, NIT > &A
	) noexcept {
		return A.m;
	}
	...
	/** \internal No implementation notes. */
	template< typename DataType, typename Coords >
	size_t nnz( const Vector< DataType, tutorial, Coords > &x ) noexcept {
	...
	/** \internal No implementation notes. */
	template< typename DataType, typename Coords >
	size_t capacity( const Vector< DataType, tutorial, Coords > &x ) noexcept {
		return internal::getCoordinates( x ).size();
	}
	...
	size_t capacity(
		const Matrix< DataType, tutorial, RIT, CIT, NIT > &A
	) noexcept {
	...
	>
	RC set(
		Vector< DataType, tutorial, Coords > &x,
		const T val,
	...
	RC set(
		Vector< DataType, tutorial, Coords > &x,
		const Vector< MaskType, tutorial, Coords > &m,
		const T val,
		const Phase &phase = EXECUTE,
		const typename std::enable_if<
...
#ifdef _DEBUG
		std::cout << "setElement (tutorial) set index " << i << " to value "
			<< internal::getRaw( x )[ i ] << "\n";
#endif
	...
	RC set(
		Vector< OutputType, tutorial, Coords > &x,
		const Vector< InputType, tutorial, Coords > &y,
		const Phase &phase = EXECUTE
	) {
		// static sanity checks
		NO_CAST_ASSERT( ( !(descr & descriptors::no_casting) ||
		...
		static_assert( !in_is_void || out_is_void,
			"grb::set (tutorial, vector <- vector, masked): "
	...
	RC set(
		Vector< OutputType, tutorial, Coords > &x,
		const Vector< MaskType, tutorial, Coords > &mask,
		const Vector< InputType, tutorial, Coords > &y,
		const Phase &phase = EXECUTE,
		...
		#pragma omp parallel
		{
			...
			internal::Coordinates< tutorial >::Update local_update =
	...
	namespace internal {
		...
		>
		RC set(
			Matrix< OutputType, tutorial > &C,
			const Matrix< InputType1, tutorial > &A,
			const InputType2 * __restrict__ id = nullptr
		) noexcept {
#ifdef _DEBUG
			std::cout << "Called grb::set (matrices, tutorial), execute phase\n";
#endif
			// static checks
			NO_CAST_ASSERT(
	...
	RC set(
		Matrix< OutputType, tutorial > &C,
		const Matrix< InputType, tutorial > &A,
		const Phase &phase = EXECUTE
	) noexcept {
...
#ifdef _DEBUG
		std::cout << "Called grb::set (matrix-to-matrix, tutorial)" << std::endl;
#endif
		// static checks
		NO_CAST_ASSERT( ( !(descr & descriptors::no_casting) ||
	...
	RC set(
		...
		static_assert( !std::is_same< OutputType, void >::value,
			...
			"matrix as output" );
#ifdef _DEBUG
		std::cout << "Called grb::set (matrix-to-value-masked, tutorial)\n";
#endif
		// static checks
		NO_CAST_ASSERT( ( !(descr & descriptors::no_casting) ||
	...
	>
	RC buildVector( Vector< InputType, tutorial, Coords > &x,
		...
		NO_CAST_ASSERT( ( !( descr & descriptors::no_casting ) ||
			...
			"grb::buildVector (tutorial implementation)",
			"Input iterator does not match output vector type while no_casting "
			"descriptor was set" );

		// in the sequential tutorial implementation, the number of user processes always equals 1
	...
	uintptr_t getID( const Vector< InputType, tutorial, Coords > &x ) {
		assert( grb::size( x ) != 0 );
		const uintptr_t ret = x._id;
#ifdef _DEBUG
		std::cerr << "In grb::getID (tutorial, vector).\n"
	...
	>
	RC wait(
		const Vector< InputType, tutorial, Coords > &x,
		const Args &... args
	) {
		(void) x;
		return wait( args... );
