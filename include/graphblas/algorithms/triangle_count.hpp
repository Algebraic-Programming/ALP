...
namespace grb {
	...
	namespace algorithms {
		...
		namespace {
			...
			grb::RC trilu( const grb::Matrix< D, nonblocking, I, J > & A,
				grb::Matrix< D, nonblocking, I, J > & L,
				grb::Matrix< D, nonblocking, I, J > & U ) {
				//
		...
		RC triangle_count_generic( size_t & count,
			Matrix< D, nonblocking, I, J > & MXM_out,
			const Matrix< D, nonblocking, I, J > & MXM_lhs,
			const Matrix< D, nonblocking, I, J > & MXM_rhs,
			Matrix< D, nonblocking, I, J > & EWA_out,
			const Matrix< D, nonblocking, I, J > & EWA_rhs,
			const D div_factor,
			...
			std::cout << "number of nonzeros in B = LU " << internal::getNonzeroCapacity( MXM_out ) << std::endl;
			...
			std::cout << "number of nonzeros in A.*B " << internal::getNonzeroCapacity( EWA_out ) << std::endl;
		...
		RC triangle_count( const TriangleCountAlgorithm algo,
			size_t & count,
			const Matrix< D, nonblocking, I, J > & A,
			Matrix< D, nonblocking, I, J > & MXM_out,
			Matrix< D, nonblocking, I, J > & EWA_out,
			Matrix< D, nonblocking, I, J > & L = { 0, 0 },
			Matrix< D, nonblocking, I, J > & U = { 0, 0 } ) {
			// Static assertions
			...
			switch( algo ) {
				...
				case TriangleCountAlgorithm::Sandia_TT: {
					...
					const Matrix< D, nonblocking, I, J > & T = ( nrows( U ) == 0 || ncols( U ) == 0 ) ? L : U;
