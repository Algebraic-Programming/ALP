...
void grbProgram( const struct input & data_in, struct output & out ) {
	...
	if( out.rep == 0 ) {
		timer.reset();
		rc = simple_pagerank< descriptors::no_operation >( pr, L, buf1, buf2, buf3, 0.85, 1e-7, 1000, &( out.iterations ), &( out.residual ) );
...
int main( int argc, char ** argv ) {
	...
	if( argc < 3 || argc > 7 ) {
		...
		std::cout << "(inner iterations) is optional, the default is " << grb::config::BENCHMARKING::inner()
			<< ". If set to zero, the program will select a number of iterations approximately required to take at least one second to complete.\n";
		...
		std::cout << "(verification <truth-file>) is optional. The <truth-file> must point to a pre-computed solution that the computed solution will be verified against." << std::endl;
		return 0;
	}
	...
	const size_t n = out.pinnedVector.length();
	...
	if( out.error_code != 0 ) {
		...
		if( verification ) {
			out.error_code = vector_verification( out.pinnedVector, truth_filename, 1e-5, 1e-6 );
			if( out.error_code == 0 ) {
