...
namespace grb {
	namespace utils {
		 ...
		 */
		template< typename T >
		RC alloc( const std::string prefix, const std::string postfix, T * __restrict__ & pointer, const size_t size, const bool shared, utils::AutoDeleter< T > & deleter ) {
			size_t allocd = 0;
			const RC ret = internal::Allocator<>::functions::alloc( allocd, pointer, size, shared, deleter );
			internal::Allocator<>::functions::postAlloc( ret, allocd, prefix, postfix );
			return ret;
		}
		 ...
		 */
		template< typename T, typename... Targs >
		RC alloc( const std::string prefix, const std::string postfix, T * __restrict__ & pointer, const size_t size, const bool shared, utils::AutoDeleter< T > & deleter, Targs &&... args ) {
			size_t allocd = 0;
