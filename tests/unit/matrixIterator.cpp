...
template< typename ValT, typename OrigIterT > RC test_matrix_iter(
	...
	bool locally_equal = utils::compare_non_zeroes< ValT >( nrows(mat),
	...
	for( auto it = orig_begin; it != orig_end; ++it ) {
		if( grb::internal::Distribution<>::global_index_to_process_id( it.i(),
			15, grb::spmd<>::nprocs() ) != spmd<>::pid() ) {
			continue;
		}
		row_count[ it.i() - row_col_offset ]++;
		col_count[ it.j() - row_col_offset ]++;
	}
	for( const NZC& nz : mat_values ) {
		row_count[ nz.i() - row_col_offset ]--;
		col_count[ nz.j() - row_col_offset ]--;
	}
	...
	return locally_equal && count == nnz( mat ) && rows_match
		&& cols_match ? SUCCESS : FAILED;
}

template< typename ValT > RC test_matrix( size_t num_nnz, const size_t* rows, const size_t* cols,
	...
	auto orig_end = utils::makeSynchronized( rows + num_nnz, cols + num_nnz, values + num_nnz, 0 );
...
void grb_program( const size_t & n, grb::RC & rc ) {
	...
	if( rc != SUCCESS ) {
		std::cerr << "\tsubtest 3 (general " << n << " x " << n << " matrix) FAILED" << std::endl;
		return;
	}

	rc = test_matrix( 15, I1, J1, offset, D );
	if( rc != SUCCESS ) {
		std::cerr << "\tsubtest 4 (diagonal pattern " << n << " x " << n << " matrix) FAILED" << std::endl;
		return;
	}
