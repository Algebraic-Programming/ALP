...
void _grb_exec_spmd( lpf_t ctx, lpf_pid_t s, lpf_pid_t P, lpf_args_t args ) {
	...
	if( broadcast && P > 1 ) {
		...
		if( s == 0 ) {
			...
			brc = lpf_register_global( ctx, const_cast< void * >( args.input ), args.input_size, &global );
		} else {
	...
	if( args.f_size == 1 ) {
		...
		grb_func_t grb_program = reinterpret_cast< grb_func_t >( args.f_symbols[ 0 ] );
		( *grb_program )( data_in, data_out );
	} else {
		// assume we are performning benchmarks
		typedef void ( *grb_func_t )( const T &, U & );
		typedef void ( *bench_func_t )( void ( *grb_program )( const T &, U & ), const T &, U &, lpf_pid_t );
...
void _grb_exec_varin_spmd( lpf_t ctx, lpf_pid_t s, lpf_pid_t P, lpf_args_t args ) {
	...
	if( args.f_size == 1 ) {
		...
		// assume we are performning benchmarks
		typedef void ( *grb_func_t )( void *, size_t, U & );
		typedef void ( *bench_func_t )( void ( *grb_program )( void *, size_t, U & ), void *, size_t, U &, lpf_pid_t );
		bench_func_t bench_program = reinterpret_cast< bench_func_t >( args.f_symbols[ 0 ] );
		grb_func_t grb_program = reinterpret_cast< grb_func_t >( args.f_symbols[ 1 ] );
...
namespace grb {
	...
	public:
		...
		Launcher( const MPI_Comm comm = MPI_COMM_WORLD ) {
			...
			if( LPF_MPI_AUTO_INITIALIZE ) {
				throw std::runtime_error( "Program was not linked with the "
										  "symbol LPF_MPI_AUTO_INITIALIZE set "
										  "to 0 while an instance of "
										  "Launcher<Manual> or "
										  "Launcher<FROM_MPI> is being "
										  "requested." );
			}
			...
			if( initrc != LPF_SUCCESS ) {
				throw std::runtime_error( "LPF could not connect launcher "
										  "group over TCP/IP." );
			}
...
#ifndef _GRB_NO_STDIO
				std::cerr << "Warning: could not destroy launcher::init from "
							 "~Launcher.\n";
#endif
			}
			init = LPF_INIT_NONE;
		...
		RC exec( void ( *grb_program )( const void *, const size_t, U & ), const void * data_in, const size_t in_size, U & data_out, const bool broadcast = false ) const {
			...
			// do hook
			const lpf_err_t spmdrc = broadcast ? lpf_hook( init, &(_grb_exec_varin_spmd< U, true >), args ) : lpf_hook( init, &(_grb_exec_varin_spmd< U, false >), args );
		...
		RC exec( void ( *grb_program )( const T &, U & ), // user GraphBLAS program
			...
			// do hook
			const lpf_err_t spmdrc = broadcast ? lpf_hook( init, &(_grb_exec_spmd< T, U, true >), args ) : lpf_hook( init, &(_grb_exec_spmd< T, U, false >), args );
	...
	private:
		...
		static_assert( mode != FROM_MPI, "EXEC_MODE::FROM_MPI for BSP1D is implemented in specialised class" );
	...
	public:
		...
		Launcher( const size_t process_id = 0,        // user process ID
			...
			if( process_id >= nprocs ) {
				throw std::invalid_argument( "Process ID must be strictly "
											 "smaller than total number of "
											 "user processes." );
			}
			...
			if( mode == MANUAL ) {
				...
				if( LPF_MPI_AUTO_INITIALIZE ) {
					throw std::runtime_error( "Program was not linked with the "
											  "symbol LPF_MPI_AUTO_INITIALIZE "
											  "set to 0 while an instance of "
											  "Launcher<Manual> or "
											  "Launcher<FROM_MPI> is being "
											  "requested." );
				}
			...
			if( mode == MANUAL ) {
				...
				if( hostname.compare( "" ) == 0 || port.compare( "" ) == 0 ) {
					throw std::invalid_argument( "Hostname and/or port name "
												 "cannot be empty." );
				}

				// try and create a lpf_init_t
				const lpf_err_t initrc = lpf_mpi_initialize_over_tcp( hostname.c_str(), port.c_str(), // server info
					120000,                                                                           // time out
		...
		~Launcher() {
			if( mode == MANUAL ) {
				...
				if( finrc != LPF_SUCCESS ) {
#ifndef _GRB_NO_STDIO
					std::cerr << "Warning: could not destroy launcher::init "
								 "from ~launcher.\n";
#endif
				}
				init = LPF_INIT_NONE;
		...
		RC exec( void ( *grb_program )( const void *, const size_t, U & ), const void * data_in, const size_t in_size, U & data_out, const bool broadcast = false ) const {
			...
			if( mode == MANUAL ) {
				// do hook
				spmdrc = broadcast ? lpf_hook( init, &(_grb_exec_varin_spmd< U, true >), args ) : lpf_hook( init, &(_grb_exec_varin_spmd< U, false >), args );
			} else {
				assert( mode == AUTOMATIC );
				// do exec
				spmdrc = lpf_exec( LPF_ROOT, LPF_MAX_P, &(_grb_exec_varin_spmd< U >), args );
			}
		...
		RC exec( void ( *grb_program )( const T &, U & ), // user GraphBLAS program
			...
			if( mode == MANUAL ) {
				// do hook
				spmdrc = broadcast ? lpf_hook( init, &(_grb_exec_spmd< T, U, true >), args ) : lpf_hook( init, &(_grb_exec_spmd< T, U, false >), args );
			} else {
				assert( mode == AUTOMATIC );
				// do exec
...
#ifndef _GRB_NO_STDIO
					std::cerr << "Warning: MPI_Finalize returned non-SUCCESS "
								 "exit code.\n";
#endif
				}
