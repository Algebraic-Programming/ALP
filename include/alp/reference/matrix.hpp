...
namespace alp {
	...
	namespace internal {
		...
		const Vector< T, reference > & getContainer( const StorageBasedMatrix< T, AmfType, requires_allocation, reference > & A );

		template< typename T, typename AmfType, bool requires_allocation >
		Vector< T, reference > & getContainer( StorageBasedMatrix< T, AmfType, requires_allocation, reference > & A );
