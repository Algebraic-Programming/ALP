...
void checkCRSandCCS( const grb::Matrix< T > & C,
	...
	for( size_t i = 0; i < n; ++i ) {
		...
		if( entries != rlens[ i ] ) {
			std::cerr << "Error: unexpected number of entries " << entries << ", expected " << rlens[ i ] << " (CRS).\n";
			rc = FAILED;
		}
		for( size_t k = crs1.col_start[ i ]; k < crs1.col_start[ i + 1 ]; ++k ) {
			if( crs1.row_index[ k ] != J[ k ] ) {
				std::cerr << "Error: unexpected entry at ( " << i << ", " << crs1.row_index[ k ] << " ), expected one at ( " << i << ", " << J[ k ] << " ) instead (CRS).\n";
				rc = FAILED;
			}
			if( crs1.values[ k ] != expect_CRS[ k ] ) {
				std::cerr << "Error: unexpected value " << crs1.values[ k ] << "; expected " << expect_CRS[ k ] << " (CRS).\n";
				rc = FAILED;
			}
		}
	}
	...
	for( size_t j = 0; j < n; ++j ) {
		...
		if( entries != clens[ j ] ) {
			std::cerr << "Error: unexpected number of entries " << entries << ", expected " << clens[ j ] << " (CCS).\n";
			rc = FAILED;
		}
		for( size_t k = ccs1.col_start[ j ]; k < ccs1.col_start[ j + 1 ]; ++k ) {
			if( ccs1.row_index[ k ] != I[ k ] ) {
				std::cerr << "Error: unexpected entry at ( " << ccs1.row_index[ k ] << ", " << j << " ), expected one at ( " << I[ k ] << ", " << j << " ) instead (CCS).\n";
				rc = FAILED;
			}
			if( ccs1.values[ k ] != expect_CCS[ k ] ) {
				std::cerr << "Error: unexpected value " << ccs1.values[ k ] << "; expected " << expect_CCS[ k ] << " (CCS).\n";
				rc = FAILED;
			}
		}
	}
...
void grbProgram( const void *, const size_t, grb::RC & rc ) {
	...
	if( rc == SUCCESS ) {
		rc = grb::buildMatrixUnique( A, I_A, J_A, V_A, nelts_A, SEQUENTIAL );
	}
	if( rc == SUCCESS ) {
		rc = grb::resize( B, nelts_B );
	}
	if( rc == SUCCESS ) {
		rc = grb::buildMatrixUnique( B, I_B, J_B, V_B, nelts_B, SEQUENTIAL );
	}
	if( rc == SUCCESS ) {
		rc = grb::resize( A_pattern, nelts_A );
	}
	if( rc == SUCCESS ) {
		rc = grb::buildMatrixUnique( A_pattern, I_A, J_A, nelts_A, SEQUENTIAL );
	}
	if( rc == SUCCESS ) {
		rc = grb::resize( B_pattern, nelts_B );
	}
	if( rc == SUCCESS ) {
		rc = grb::buildMatrixUnique( B_pattern, I_B, J_B, nelts_B, SEQUENTIAL );
	}
	if( rc != SUCCESS ) {
	...
	std::cout << "\tVerifying the monoid version of mxm_elementwise, A and B "
				 "value matrices\n";
	rc = grb::mxm_elementwise( C, A, B, mulmono );
	if( rc != SUCCESS ) {
		std::cerr << "Call to grb::mxm_elementwise FAILED\n";
		return;
	}
	...
	std::cout << "\tVerifying the monoid version of mxm_elementwise, A value "
				 "matrix, B pattern matrix \n";
	rc = grb::mxm_elementwise( C, A, B_pattern, mulmono );
	if( rc != SUCCESS ) {
		std::cerr << "Call to grb::mxm_elementwise FAILED\n";
		return;
	}
	...
	std::cout << "\tVerifying the monoid version of mxm_elementwise, A pattern "
				 "matrix, B value matrix \n";
	rc = grb::mxm_elementwise( C, A_pattern, B, mulmono );
	if( rc != SUCCESS ) {
		std::cerr << "Call to grb::mxm_elementwise FAILED\n";
		return;
	}
	...
	std::cout << "\tVerifying the monoid version of mxm_elementwise, A pattern "
				 "matrix, B pattern matrix \n";
	rc = grb::mxm_elementwise( C, A_pattern, B_pattern, mulmono );
	if( rc != SUCCESS ) {
		std::cerr << "Call to grb::mxm_elementwise FAILED\n";
		return;
	}
	...
	std::cout << "\tVerifying the operator version of mxm_elementwise (only "
				 "value matrices) \n";
	rc = grb::mxm_elementwise( C, A, B, mulmono.getOperator() );
	if( rc != SUCCESS ) {
		std::cerr << "Call to grb::mxm_elementwise FAILED\n";
		return;
	}
