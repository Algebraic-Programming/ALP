	...
	namespace algorithms
	{
		...
		RC pLaplacian_multi(
			...
			arma::sp_mat A = arma::sp_mat(n, n);
			{
				...
				arma::sp_mat B = arma::sp_mat(n, n);
				bool diag_empty = true;
				bool diag_zero = true;

				// ingest adjacency into B, keep track of diagonal
				for( const auto &triplet : W ) {
					if( diag_empty && triplet.first.first == triplet.first.second ) {
						diag_empty = false;
					}
					if( diag_zero && triplet.first.first == triplet.first.second && triplet.second != 0 ) {
						diag_zero = false;
					}
					B(triplet.first.first, triplet.first.second) = triplet.second;
				...
				if( !B.is_symmetric() ) {
					std::cerr << "Error: adjacency must be symmetric" << std::endl;
					return ILLEGAL;
				}
				// detect if workaround required or not
				std::cout << "\tworkaround check..." << std::endl;
				bool add_id = false;
				if( diag_empty ) {
					std::cerr << "Warning: empty adjacency diagonal, which may make Armadillo "
						"unhappy. Work around by adding an identity matrix to the adjacency "
						"matrix when building the graph Laplacian.\n";
					add_id = true;
				} else if( diag_zero ) {
					std::cerr << "Warning: only diagonal entries have exact value zero, which "
						"may make Armadillo unhappy. Work around by adding an identity matrix "
						"to the adjacency matrix when building the graph Laplacian.\n";
					add_id = true;
				}
				// A = D(B) - W(B)
				std::cout << "\tbuilding graph Laplacian..." << std::endl;
				if( add_id ) {
					std::cout << "Adding an identity matrix to the adjacency matrix while "
						"building the graph Laplacian.\n";

					arma::sp_mat eye = arma::sp_mat(n, n);
					eye.eye();
					A = diagmat( sum( B ) ) - B + eye;
				} else {
					std::cout << "Building the graph Laplacian...\n";
					A = diagmat( sum( B ) ) - B;
				}
			}
