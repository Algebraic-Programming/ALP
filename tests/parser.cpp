 ...
 */
bool readEdges( const std::string filename, const bool use_indirect, size_t * const n, size_t * const nz, size_t ** const I, size_t ** const J, double ** const weights ) {
...
int main( int argc, char ** argv ) {
	...
	/* The below tests for automatic derivation of number of vertices, but this is not supported by the SNAP cit-HepTh data file
	 * n = SIZE_MAX;
	...
	for( size_t k = 0; ret == 0 && k < nz; ++k, ++synced_it ) {
		if( I[ k ] != synced_it.i() ) {
			std::cerr << "Synchronised file iterator has mismatching row "
						 "indices at position "
					  << k << ": read " << synced_it.i() << " instead of " << I[ k ] << "\n";
			ret = 10;
		}
		if( J[ k ] != synced_it.j() ) {
			std::cerr << "Synchronised file iterator has mismatching column "
						 "indices at position "
	...
	if( nz != citHepTh.nz() ) {
		std::cerr << "Util parsers imported into std::map< size_t, std::set< "
					 "size_t > > changes nonzero count ( "
				  << nz << " versus " << citHepTh.nz() << " ).\n";
		ret = 20;
	}
	...
	if( citHepTh.filename() != citHepTh2.filename() || citHepTh.m() != citHepTh2.m() || citHepTh.n() != citHepTh2.n() || citHepTh.nz() != citHepTh2.nz() ||
		citHepTh.isPattern() != citHepTh2.isPattern() || citHepTh.isSymmetric() != citHepTh2.isSymmetric() || citHepTh.usesDirectAddressing() != citHepTh2.usesDirectAddressing() ) {
	...
	if( citHepTh.rowMap().size() != citHepTh2.rowMap().size() ) {
		std::cerr << "Inferred matrix and explicit matrix row maps are not of "
					 "equal size ("
				  << citHepTh.rowMap().size() << " vs. " << citHepTh2.rowMap().size() << ").\n";
		ret = 33;
	};
	if( citHepTh.colMap().size() != citHepTh2.colMap().size() ) {
		std::cerr << "Inferred matrix and explicit matrix col maps are not of "
					 "equal size ("
	...
	if( nz != citHepTh.nz() ) {
		std::cerr << "Inferred matrix does not contain all nonzeroes found in "
					 "the explicit matrix.\n";
		ret = 40;
	}

// the below test is illegal since reordering of input changes indirect mappig
#if 0
