...
#ifndef _H_HPCG_MATRIX_BUILDING_UTILS
#define _H_HPCG_MATRIX_BUILDING_UTILS
...
grb::RC build_ndims_system_matrix( grb::Matrix< T, B > & M, const std::array< std::size_t, DIMS > & sys_sizes,
		std::size_t halo_size, T diag_value, T non_diag_value ) {
	static_assert( DIMS > 0, "DIMS must be > 0" );
	const std::size_t n { std::accumulate( sys_sizes.cbegin(), sys_sizes.cend(), 1UL, std::multiplies< std::size_t >() ) };
	...
	// round ratio up
	std::size_t per_node_elements { (n + grb::spmd<>::nprocs() - 1 ) / grb::spmd<>::nprocs() };
	const std::size_t start_row { grb::spmd<>::pid() * per_node_elements };
	const std::size_t end_row { std::min( start_row + per_node_elements, n ) };
	grb::utils::matrix_generator_iterator< DIMS, T > begin( sys_sizes, start_row, halo_size, diag_value, non_diag_value );
	grb::utils::matrix_generator_iterator< DIMS, T > end( sys_sizes, end_row, halo_size, diag_value, non_diag_value );
...
grb::RC build_ndims_coarsener_matrix( grb::Matrix< T, B > & M, const std::array< std::size_t, DIMS > & coarser_sizes, const std::array< std::size_t, DIMS > & finer_sizes ) {
	...
	grb::utils::coarsener_generator_iterator< DIMS, T > begin( coarser_sizes, finer_sizes, 0 );
	grb::utils::coarsener_generator_iterator< DIMS, T > end( coarser_sizes, finer_sizes, rows );
...
#endif // _H_HPCG_MATRIX_BUILDING_UTILS
