...
void grb_program( const size_t & n, grb::RC & rc ) {
	...
	{
		grb::Vector< double > tempVector( n );
		rc = grb::set( tempVector, 1.5 );
		if( rc != SUCCESS ) {
			std::cerr << "\tinitialisation FAILED\n";
			return;
		}
		if( grb::nnz( tempVector ) != n ) {
			std::cerr << "\tinitialisation FAILED: vector has " << grb::nnz( tempVector ) << " entries, while expecting " << n << "\n";
			rc = FAILED;
			return;
		}
		vector = std::move( tempVector );
	}
	if( grb::nnz( vector ) != n ) {
		std::cerr << "\tunexpected number of nonzeroes " << grb::nnz( vector ) << ", expected " << n << "\n";
		rc = FAILED;
	}
