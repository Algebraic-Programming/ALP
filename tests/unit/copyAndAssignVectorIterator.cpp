...
void grb_program( const size_t & n, grb::RC & rc ) {
		...
		for( ; iterator != end_copy; ++iterator, ++count ) {
			...
			if( pair.second != 1.5 ) {
				std::cerr << "\tunexpected entry ( " << pair.first << ", " << pair.second
					<< " ), expected value 1.5 after copy-constructing iterators\n";
				rc = FAILED;
			}
		}
		grb::collectives<>::allreduce( count, grb::operators::add< size_t >() );
		if( count != n ) {
			std::cerr << "\tunexpected number of nonzeroes " << count << ", expected " << n
				<< " after copy-constructing iterators\n";
			rc = FAILED;
		}
	} catch( ... ) {
	...
	GRB_UTIL_IGNORE_MAYBE_UNINITIALIZED // GCC 8.5.0 signals the below iterator will be used unitialised, but this cannot happen
	                                    // because start, once created, so far is never used-- only copied. Here it is moved to
					    // the `iterator' field, and then finally, for the first time, used. In other words, the
					    // only way uninitialized use would occur is if the call to vector.cbegin() above had
					    // failed.
	auto iterator = std::move( start );
	GRB_UTIL_RESTORE_WARNINGS           // restore all compiler warnings
