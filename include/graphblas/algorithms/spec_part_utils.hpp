...
/*
* @author Verner Vlacic, Huawei Zurich Research Center
* @date 25 Feb 2021
*/
...
namespace grb {
	...
	IOType p_norm(
		...
		// (the copy happens in the function signature)
		grb::RC rc = grb::eWiseLambda( [&p, &x](const size_t i) {
				x[i] = std::pow(std::abs(x[i], p));
			}, x );
		//The below makes use of an unofficially supported primitive instead:
		/*eWiseMap([&p](const IOType u){
			return std::pow(std::abs(u),p);
		}, x );*/

		rc = rc ? rc : grb::foldl(ret,x,mono_add);
		if( rc != grb::SUCCESS ) {
	...
	namespace algorithms{
		...
		namespace spec_part_utils{
			...
			//FIXME this function is not auto-parallelised
			...
			RC apply_Laplacian(
				...
				//FIXME for performance, diag_x and Ax can be workspace arguments to this function
				Vector<IOType> diag_x(n), Ax(n);
				grb::RC rc = grb::set(diag_x,0);
				rc = rc ? rc : grb::set(Ax,0);
				rc = rc ? rc : grb::eWiseApply( diag_x, diag, x, operators::mul<IOType>() ); //CHANGE TO apply ONCE ALBERT-JAN RENAMES eWiseApply
				rc = rc ? rc : grb::mxv( Ax, A, x, pattern_ring );
				rc = rc ? rc : grb::eWiseApply( Lx, diag_x, Ax, operators::subtract<IOType>() );
				return rc;
			}

			//generalized "rounding function", can be used to obtain a 0-1 partition according
			//to the signs of vector, or to implement the elementwise sign function
			template <
				typename IOType,
			...
			RC general_rounding(
				...
				if( grb::nnz( par ) != grb::size( par ) ) {
					return ILLEGAL;
				}
				return eWiseLambda([ &x, &par, &up_val, &down_val ]( const size_t i ) {
					if ( x[i] >= 0 ){
						par[ i ] = static_cast<IntegerT>(up_val);
					}
					else{
						par[ i ] = static_cast<IntegerT>(down_val);
					}
				}, x, par );
			}
			...
			RC ratio_cheeger_cut(
				IOType &cut,
				const grb::Vector<IntegerT> &par,
				const grb::Matrix<IntegerT> &A,
				const size_t m,
				const size_t n,
				const grb::Semiring<
			...
			> &integers_ring
				...
				//compute edges=Ax
				//FIXME the below could be a workspace argument to this function
				grb::Vector<IntegerT> edges(m);

				grb::RC rc = grb::mxv( edges, A, par, integers_ring );
			...
			RC phi_p( Vector<IOType> &x, const pType &p ){
				return grb::eWiseLambda( [&x,&p](const size_t i) {
						x[ i ] = phi_p_scalar( x[ i ], p );
					}, x );
				//The below makes use of an unofficially supported primitive instead:
				/*return eWiseMap( [ &x, &p ]( const IOType u ){
			...
			RC phi_p_normalize(
				...
				//FIXME this is not auto-parallelised
				...
				grb::RC rc = phi_p( x, p );
				// maybe here?!!
				rc = rc ? rc : foldl( x, -to_subtract, mono_add );

				rc = rc ? rc : phi_p( x, p/(p-1) );
			...
			RC RCut(
				...
				// FIXME the below could be a workspace argument of this function instead
				...
				grb::RC rc = grb::set( ones, true );
				rc = rc ? rc : grb::vxm( degs, ones, W, pattern_sum );

				// parallelise this once we have random-access iterators
				// FIXME see above (currently not auto-parallelised)
				for ( const auto &pair : x ) {
					// no shortcut based on rc != SUCCESS because of the overarching non-parallel issue
					rc = rc ? rc : grb::setElement( *(cluster_indic[ pair.second ]), true, pair.first );
				}
				if( rc != grb::SUCCESS ) {
					throw std::runtime_error( grb::toString( rc ) );
				}
				...
				rcut = 0;
				// FIXME move up and/or into workspace arguments
				...
				for( size_t i = 0; i < k; ++i ) {
					...
					// W part
					rc = grb::clear( aux );
					rc = rc ? rc : grb::set(aux,0);
					rc = rc ? rc : grb::vxm( aux, *(cluster_indic[ i ]), W, pattern_sum );

					rc = rc ? rc : eWiseLambda( [ &cluster_indic, &aux, &i ] ( size_t j ){
						//std::cout << (*(cluster_indic[ i ]))[ j ] << " " << aux[ j ] << "\n";
					}, (*(cluster_indic[ i ])) );

					rc = rc ? rc : grb::dot( to_add, *(cluster_indic[ i ]), aux, pattern_sum );
					//to_add = - to_add;
					...
					rc = rc ? rc : grb::dot( to_add_2, *(cluster_indic[ i ]), degs, pattern_sum );
				...
				for ( size_t i = 0; i < k && rc == grb::SUCCESS; ++i ) {
					...
					rc = rc ? rc : grb::set(*cluster_indic[i], false);
			 ...
			 *
			 * @param[out] A resulting incidence matrix
			 * @param[in] A_hyper Hypergraph matrix
			 * @parampin] x At which vector
			 */
			template < typename IOType >
			RC HyperToIncid(
				Matrix< IOType > &A,
				const Matrix< IOType > &A_hyper,
				const Vector< IOType > &x
			) {
				...
				// add labels to x
				grb::Vector< size_t > labels( n );
				ret = ret ? ret : grb::set< grb::descriptors::use_index >( labels, 0 );

				// FIX THIS
				// declare vector of selected vertices
				// FIXME the below containers could be a workspace argument of this function
				grb::Vector< indexIOType > max_selected( m ), min_selected( m );
				// FIXME the below code cannot be auto-parallelised
			...
			RC RCutAdj(
				...
				// FIXME the below could be moved to a workspace argument
				std::vector< grb::Vector< double > * > cluster_indic( k );
				for ( size_t i = 0; i < k && rc == grb::SUCCESS; ++i ) {
					cluster_indic[ i ] = new grb::Vector< double >( n );
					rc = grb::set( *(cluster_indic[ i ]), 0 );
				}

				// parallelise this once we have random-access iterators
				// FIXME the below could be moved to a workspace argument
				std::vector< size_t > sizes( k, 0 );
				// FIXME the below loop cannot auto-parallelise
				for ( const auto &pair : x ) {
					rc = rc ? rc : grb::setElement( *(cluster_indic[ pair.second ]), 1.0, pair.first );
					++sizes[ pair.second ];
				}
				...
				for ( size_t i = 0; i < k && rc == grb::SUCCESS; ++i ) {
					...
					// A part
					rc = HyperToIncid( A, A_hyper, *(cluster_indic[ i ]) );

					rc = rc ? rc : grb::clear( aux );
					rc = rc ? rc : grb::set( aux, 0 );
					rc = rc ? rc : grb::mxv( aux, A, *(cluster_indic[ i ]), reals_ring );

					rc = rc ? rc : grb::eWiseLambda( [&aux]( const size_t i ) {
							aux[ i ] = std::fabs( aux[ i ] );
						}, aux );
					// The below uses a non-standard primitive instead
					/*rc = rc ? rc : grb::eWiseMap( [] ( double &u ){
						return std::fabs( u );
					}, aux );*/

					rc = rc ? rc : grb::foldl( to_add, aux, reals_ring.getAdditiveMonoid() );
				...
				// FIXME the below, on errors, may like
				for ( size_t i = 0; i < k && rc == grb::SUCCESS; ++i ) {
					delete cluster_indic[i];
				}
			...
			*
			* @param[in] A_hyper Hyper-incidence matrix
			* @param[in,out] Eigs Contains the initial guess for the k eigenvectors and the final k eigenvectors, should be nonzero
			* @param[in] Cj Constant for offsetting the pLaplacian
			* @param[in] conv Convergence tolerance
			*/
			template < typename IOType >
			RC PowerIter(
				const Matrix< IOType > &A_hyper, //hyper-incidence matrix
				double p,
				...
				// working vectors
				// FIXME these could be workspace arguments to this function
				...
				for ( size_t j = 0; j < k && ret == grb::SUCCESS; j++ ) {
					...
					do {
						...
						for ( size_t l = 0; l < j && ret == grb::SUCCESS; ++l ) {
						// if ( j > 0 ) {
							IOType innerprod = 0.0;
						...
						// quit early to avoid side effects
						if( ret != grb::SUCCESS ) {
							return ret;
						}
