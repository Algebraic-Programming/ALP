...
namespace alp {
	...
	class IMF {
		public:
			...
			size_t map(size_t i) {
				return static_cast< Specialization * >( this )->map();
			}

			template< typename OTHER_IMF >
			bool isSame( const OTHER_IMF & other ) const {
				static_assert( std::is_same< decltype( *this ), decltype( other ) >::value );
				return n == other.n && N == other.N
					&& static_cast< Specialization * >( this )->isSame();
			}
	...
	class Strided: public IMF< Strided > {
		...
		public:
			...
			bool isSame( const IMF & other ) const {
				return b == static_cast< const Strided & >( other ).b
					&& s == static_cast< const Strided & >( other ).s;
			}
	};

	class Select: public IMF< Select > {
		...
		public:
			...
			bool isSame( const IMF & other ) const {
				return select == static_cast< const Select & >( other ).select;
			}
	};
	...
	class Composed: public IMF< Composed > {
