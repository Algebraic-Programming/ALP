...
namespace grb {
	...
	namespace utils {
		...
		namespace internal {
			...
			/** TODO documentation */
			template< typename T, typename V, typename SR = grb::config::RowIndexType, typename SC = grb::config::ColIndexType, Backend backend = grb::config::default_backend >
			...
			protected:
				...
				const std::function< OutputType( const size_t &, const V & ) > * const converter;

				/** Main constructor */
				MatrixVectorIteratorBase( InputIteratorType vec_iter, const std::function< OutputType( const size_t &, const V & ) > & conv ) noexcept :
					vector_iterator( std::move( vec_iter ) ), initialized( false ), converter( &conv ) {}

				/** Copy constructor */
				MatrixVectorIteratorBase( const MatrixVectorIteratorBase & other ) noexcept :
					vector_iterator( other.vector_iterator ), current( other.current ), initialized( other.initialized ), converter( other.converter ) {}

				/** Move constructor */
				MatrixVectorIteratorBase( MatrixVectorIteratorBase && other ) noexcept :
					vector_iterator( std::move( other.vector_iterator ) ), current( std::move( other.current ) ), initialized( std::move( other.initialized ) ),
					converter( std::move( other.converter ) ) {}

				/** Default constructor */
				MatrixVectorIteratorBase() noexcept : vector_iterator( Vector< V, backend >().cbegin() ), initialized( false ), converter( NULL ) {}
			...
			protected:
				...
				/** Main constructor */
				MatrixVectorIteratorBase( typename Vector< V, backend >::const_iterator vec_iter, const std::function< OutputType( const size_t &, const V & ) > & conv ) noexcept :
					vector_iterator( std::move( vec_iter ) ), initialized( false ), converter( &conv ) {}
				...
				/** Default constructor */
				MatrixVectorIteratorBase() noexcept : vector_iterator( InputIteratorType() ), initialized( false ), converter( NULL ) {}
		...
		/** * TODO documentation */
		template< typename T, typename V, typename SR = grb::config::RowIndexType, typename SC = grb::config::ColIndexType, Backend backend = grb::config::default_backend >
		class MatrixVectorIterator : public internal::MatrixVectorIteratorBase< T, V, SR, SC, backend > {
		...
		public:
			...
			/** main constructor */
			MatrixVectorIterator( typename Vector< V, backend >::const_iterator vec_iter, const std::function< OutputType( const size_t &, const V & ) > & conv ) noexcept :
				internal::MatrixVectorIteratorBase< T, V, SR, SC, backend >( std::move( vec_iter ), conv ) {}

			/** default constructor */
			MatrixVectorIterator() noexcept : internal::MatrixVectorIteratorBase< T, V, SR, SC, backend >() {}

			/** copy constructor */
			MatrixVectorIterator( const MatrixVectorIterator & other ) noexcept : internal::MatrixVectorIteratorBase< T, V, SR, SC, backend >( other ) {}

			/** move constructor */
			MatrixVectorIterator( MatrixVectorIterator && other ) noexcept : internal::MatrixVectorIteratorBase< T, V, SR, SC, backend >( std::move( other ) ) {}
		...
		namespace internal {
			...
			protected:
				...
				/** Main constructor */
				VectorToMatrixConverterBase( const Vector< V, backend > & vec, std::function< OutputType( const size_t &, const V & ) > conv ) noexcept :
					src_start( std::move( vec.cbegin() ) ), src_end( std::move( vec.cend() ) ), converter( std::move( conv ) ) {}
			};

			// vector to matrix converter base -- pattern matrix specialisation
			template< typename V, typename SR, typename SC, Backend backend >
			class VectorToMatrixConverterBase< void, V, SR, SC, backend > {
		...
		/** TODO documentation */
		template< typename T, typename V, typename SR = grb::config::RowIndexType, typename SC = grb::config::ColIndexType, Backend backend = grb::config::default_backend >
		class VectorToMatrixConverter : internal::VectorToMatrixConverterBase< T, V, SR, SC, backend > {

			using typename internal::VectorToMatrixConverterBase< T, V, SR, SC, backend >::OutputType;
			using internal::VectorToMatrixConverterBase< T, V, SR, SC, backend >::src_start;
			using internal::VectorToMatrixConverterBase< T, V, SR, SC, backend >::src_end;
			using internal::VectorToMatrixConverterBase< T, V, SR, SC, backend >::converter;

		public:
			/** TODO documentation */
			VectorToMatrixConverter( const Vector< V, backend > & vec, const std::function< OutputType( const size_t &, const V & ) > & conv ) noexcept :
				internal::VectorToMatrixConverterBase< T, V, SR, SC, backend >( vec, conv ) {}

			/**
			 * Does not throw exceptions if and only if the converter does not throw.
			 */
			MatrixVectorIterator< T, V, SR, SC, backend > cbegin() const {
				return MatrixVectorIterator< T, V, SR, SC, backend >( src_start, converter );
			}
			...
			MatrixVectorIterator< T, V, SR, SC, backend > cend() const {
				return MatrixVectorIterator< T, V, SR, SC, backend >( src_end, converter );
			}
		...
		/** TODO documentation */
		template< typename OutputType, typename InputType, Backend backend, class Converter >
		VectorToMatrixConverter< OutputType, InputType > makeVectorToMatrixConverter( const grb::Vector< InputType, backend > & vec, Converter converter ) noexcept {
			return VectorToMatrixConverter< OutputType, InputType >( vec, std::move( converter ) );
		}
