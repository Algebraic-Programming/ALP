...
namespace grb {
	...
	namespace internal {
		...
		std::vector< size_t > & getPrefixSumTiles(
			...
			return A.prefix_sum_tiles;
		}		

		template< typename D, typename RIT, typename CIT, typename NIT >
		const bool & getStatusNnzTiles(
			const grb::Matrix< D, nonblocking, RIT, CIT, NIT > &A
		) noexcept {
			return A.is_nnz_tiles_written;
		}

		template< typename D, typename RIT, typename CIT, typename NIT >
		void setStatusNnzTiles(
			grb::Matrix< D, nonblocking, RIT, CIT, NIT > &A, 
			const bool status_tiles
		) noexcept {
			A.is_nnz_tiles_written = status_tiles;
		}

		template< typename D, typename RIT, typename CIT, typename NIT >
		const bool & getStatusPrefixTiles(
			const grb::Matrix< D, nonblocking, RIT, CIT, NIT > &A
		) noexcept {
			return A.is_prefix_nnz_tiles_written;
		}

		template< typename D, typename RIT, typename CIT, typename NIT >
		void setStatusPrefixTiles(
			grb::Matrix< D, nonblocking, RIT, CIT, NIT > &A, 
			const bool status_prefix
		) noexcept {
			A.is_prefix_nnz_tiles_written = status_prefix;
		}

		template< typename D, typename RIT, typename CIT, typename NIT >
		const size_t & getNumTilesNonblocking(
			const grb::Matrix< D, nonblocking, RIT, CIT, NIT > &A
		) noexcept {
			return A.num_tiles;
		}
		...
		void getCoordinatesTiles(
			...
			coorArr = A.coordinates_tiles_column[coordinates_id].coorArr;
			coorBuf = A.coordinates_tiles_column[coordinates_id].coorBuf;
...
#ifdef _H_GRB_NONBLOCKING_OMP_MATRIX
	...
	class Matrix< D, nonblocking, RowIndexType, ColIndexType, NonzeroIndexType > {
		...
		template< typename InputType, typename RIT, typename CIT, typename NIT >
		friend std::vector< size_t > & internal::getPrefixSumTiles( grb::Matrix< InputType, nonblocking, RIT, CIT, NIT > & ) noexcept;

		template< typename InputType, typename RIT, typename CIT, typename NIT >
		friend const bool & internal::getStatusNnzTiles( const grb::Matrix< InputType, nonblocking, RIT, CIT, NIT > & ) noexcept;

		template< typename InputType, typename RIT, typename CIT, typename NIT >
		friend void internal::setStatusNnzTiles( grb::Matrix< InputType, nonblocking, RIT, CIT, NIT > &, const bool ) noexcept;

		template< typename InputType, typename RIT, typename CIT, typename NIT >
		friend const bool & internal::getStatusPrefixTiles(
			const grb::Matrix< InputType, nonblocking, RIT, CIT, NIT > & ) noexcept;

		template< typename InputType, typename RIT, typename CIT, typename NIT >
		friend void internal::setStatusPrefixTiles( grb::Matrix< InputType, nonblocking, RIT, CIT, NIT > &, const bool ) noexcept;

		template< typename InputType, typename RIT, typename CIT, typename NIT >
		friend const size_t & internal::getNumTilesNonblocking( const grb::Matrix< InputType, nonblocking, RIT, CIT, NIT > & ) noexcept;
		...
		private:
			...
			/** Array that stores the local nnz for each tile the matrix is split into. Use for nonblicking execution.*/
			std::vector< size_t > nnz_tiles;
			...
			void initialize(
				...
				const size_t size_last_tile = rows % tile_size;
				if(size_last_tile > 0 )
				{
					num_tiles++;
				}
		...
		public:
			...
			~Matrix() {
#ifdef _DEBUG
				std::cerr << "In ~Matrix (nonblocking)\n"
					<< "\t matrix is " << m << " by " << n << "\n"
					<< "\t capacity is " << cap << "\n"
					<< "\t ID is " << id << "\n";
#endif
...
#ifdef _DEBUG
				std::cout << "In grb::Matrix<T,nonblocking>::cbegin\n";
#endif
	...
	/********************************************************************/
	/**
	 * A GraphBLAS matrix, nonblocking implementation. VOID SPECIALIZAITON
	 *
	 * Uses Compressed Column Storage (CCS) plus Compressed Row Storage (CRS).
	 *
	 * \warning This implementation prefers speed over memory efficiency.
	...
	template<
		...
		typename NonzeroIndexType
	>
	class Matrix< void, nonblocking, RowIndexType, ColIndexType, NonzeroIndexType > {

		static_assert( !grb::is_object< void >::value,
			"Cannot create an ALP matrix of ALP objects!" );
		...
		friend internal::Compressed_Storage< void, RowIndexType, NonzeroIndexType > &
		internal::getCRS<>(
			Matrix<
				void, nonblocking,
				RowIndexType, ColIndexType, NonzeroIndexType
		...
		friend internal::Compressed_Storage< void, ColIndexType, NonzeroIndexType > &
		internal::getCCS<>(
			Matrix<
				void, nonblocking,
				RowIndexType, ColIndexType, NonzeroIndexType
		...
		friend const internal::Compressed_Storage<
			void, ColIndexType, NonzeroIndexType
		> & internal::getCCS<>(
			const Matrix<
				void, nonblocking,
				RowIndexType, ColIndexType, NonzeroIndexType
		...
		friend const grb::Matrix<
			...
			ColIndexType, ColIndexType, NonzeroIndexType
		>
		internal::wrapCRSMatrix< void, ColIndexType, NonzeroIndexType, nonblocking >(
		...
		private:
			...
			/** The Row Compressed Storage */
			class internal::Compressed_Storage< void, RowIndexType, NonzeroIndexType > CRS;

			/** The Column Compressed Storage */
			class internal::Compressed_Storage< void, ColIndexType, NonzeroIndexType > CCS;
			 ...
			 */
			Matrix(
				const void *__restrict__ const _values,
				const ColIndexType *__restrict__ const _column_indices,
				const NonzeroIndexType *__restrict__ const _offset_array,
				const size_t _m, const size_t _n,
				const size_t _cap,
				char *__restrict__ const buf1 = nullptr,
				char *__restrict__ const buf2 = nullptr,
				void *__restrict__ const buf3 = nullptr
			) :
				id( std::numeric_limits< uintptr_t >::max() ), remove_id( false ),
				m( _m ), n( _n ), cap( _cap ), nz( _offset_array[ _m ] ),
				coorArr{ nullptr, buf1 }, coorBuf{ nullptr, buf2 },
			...
			void initialize(
				...
				if( rows > 0 && cols > 0 ) {
					...
					sizes[ 2 ] = internal::Coordinates< nonblocking >::bufferSize( rows );
					sizes[ 3 ] = internal::Coordinates< nonblocking >::bufferSize( cols );
					sizes[ 4 ] = rows * internal::SizeOf< void >::value;
					sizes[ 5 ] = cols * internal::SizeOf< void >::value;
					...
					*/
					...
					const size_t sizes[ 2 ] = {
						rows * internal::SizeOf< void >::value,
						cols * internal::SizeOf< void >::value
					};
					coorArr[ 0 ] = coorArr[ 1 ] = nullptr;
					coorBuf[ 0 ] = coorBuf[ 1 ] = nullptr;
					alloc_ok = utils::alloc(
				...
				valbuf[ 0 ] = reinterpret_cast< void * >( alloc[ 6 ] );
				valbuf[ 1 ] = reinterpret_cast< void * >( alloc[ 7 ] );
				if( m > 0 && n > 0 ) {
					CRS.replaceStart( alloc[ 0 ] );
					CCS.replaceStart( alloc[ 1 ] );
					CRS.replace( alloc[ 2 ], alloc[ 3 ] );
			...
			RC buildMatrixUnique(
				...
				// all on the same machine
				(void) mode;
				static_assert( utils::is_alp_matrix_iterator< void, InputIterator >::value,
					"the given iterator is not a valid input iterator, "
					"see the ALP specification for input iterators"
				);
				typename std::iterator_traits< InputIterator >::iterator_category category;
		...
		public:
			...
			/** The iterator type over matrices of this type. */
			typedef typename internal::Compressed_Storage<
				void, RowIndexType, NonzeroIndexType
			>::template ConstIterator<
				internal::Distribution< reference >
			> const_iterator;
