	...
	namespace internal {
		...
		class BSP1D_Data {
			...
			private:
				...
				/** Number of collective calls supported. */
				size_t coll_call_capacity;

				/** Number of bytes supported for collective reduction. */
				size_t coll_reduction_bsize;

				/** Number of bytes supported for other collectives. */
				size_t coll_other_bsize;
			...
			public:
...
#ifdef _DEBUG
					std::cout << "checkBufferSize: current size is " << buffer_size << ", "
				 ...
				 *                 of the library has become undefined.
				 */
				RC ensureBSMPCapacity(
					const size_t tag_size,
					const size_t payload_size = 0,
					const size_t num_messages = 0
				);

				/**
				 * A call to this function may re-initialise the LPF collectives to fit the
				 * given requested capacities.
				 *
				 * @param[in] requested_call_capacity   How many consecutive LPF collective
				 *                                      calls may occur within one
				 *                                      superstep.
				 * @param[in] requested_reduction_bsize The maximum element size, in bytes,
				 *                                      of elements subject to an LPF
				 *                                      reduction call.
				 * @param[in] requested_other_bsize     The maximum element size, in bytes,
				 *                                      of elements subject to any other
				 *                                      LPF collectivess call.
				 */
				RC ensureCollectivesCapacity(
				...
				inline T * getBuffer( const size_t offset = 0 ) {
					return reinterpret_cast< T * __restrict__ >(
