...
#ifndef _H_GRB_TUTORIAL_COORDINATES
#define _H_GRB_TUTORIAL_COORDINATES
...
namespace grb {
	...
	namespace internal {
		 ...
		 * coordinates. Its use is internal via, e.g., grb::Vector< T, tutorial, C >.
		 * All functions needed to rebuild or update sparsity information are
		 * encapsulated here.
		 */
		template<>
		class Coordinates< tutorial > {
...
#ifdef _H_GRB_tutorial_OMP_COORDINATES
				/**
			...
			public:
				...
				static inline size_t parbufSize( const size_t n ) noexcept {
					return config::IMPLEMENTATION< tutorial >::vectorBufferSize( n,
#ifdef _H_GRB_tutorial_OMP_COORDINATES
						   config::OMP::threads()
#else
						   1
				...
				void set(
					...
					_n = 0;
					_cap = dim;
					_buf = config::IMPLEMENTATION< tutorial >::vectorBufferSize(
						_cap,
#ifdef _H_GRB_tutorial_OMP_COORDINATES
						config::OMP::threads()
#else
				...
				RC rebuildFromStack(
					...
					if( _assigned == nullptr && _cap > 0 && _n == _cap ) {
						std::cerr << "Coordinates< tutorial >::rebuildFromStack called from a "
...
#ifdef _DEBUG
					std::cout << "Entering Coordinates::rebuildFromStack (tutorial backend, non-void version). New stack count: " << new_nz << ".\n";
...
#ifdef _DEBUG
					std::cout << "Called Coordinates::packValues (tutorial backend, non-void version)\n";
#endif
				...
				inline bool joinUpdate( Update &update ) noexcept {
					...
					StackType * __restrict__ const pfBuf = _buffer +
						config::IMPLEMENTATION< tutorial >::vectorBufferSize( _cap, T );
...
/*#ifdef _DEBUG
				...
				inline void prefetch_assigned( const size_t i ) const noexcept {
					assert( i < _cap + config::PREFETCHING< tutorial >::distance() );
...
#endif // end `_H_GRB_TUTORIAL_COORDINATES'
