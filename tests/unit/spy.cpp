...
void grb_program(
	const void * const fn_p, const size_t fn_length,
	grb::RC &rc
) {
	...
	rc = grb::buildMatrixUnique(
		original, reader.cbegin(), reader.cend(), grb::PARALLEL );
	if( rc != grb::SUCCESS ) {
	...
	if( rc == grb::SUCCESS && grb::spmd<>::pid() == 0 ) {
		std::cout << "Spy matrix of " << p << " by " << q << " pixels has "
			<< grb::nnz( spy ) << " nonzeroes, versus " << grb::nnz( original )
			<< " nonzeroes in the original " << m << " by " << n << " matrix\n";
	}
	for( const auto &nonzero : spy ) {
		if( nonzero.second <= 0 ) {
			std::cout << "Invalid entry at spy( " << nonzero.first.first << ", "
				<< nonzero.first.second << " ): " << nonzero.second
				<< ", expected something strictly larger than 0\n";
			rc = grb::FAILED;
		}
	}

	// check if call to spy with normalize=true succeeds and yields a consistent
	// number of nonzeroes
	grb::Matrix< double > spy2( p, q );
	rc = rc ? rc : grb::algorithms::spy< true >( spy2, original );
	if( rc == grb::SUCCESS && grb::nnz( spy ) != grb::nnz( spy2 ) ) {
		std::cerr << "Unexpected number of nonzeroes for spy2: " << grb::nnz(spy2)
			<< ", expected " << grb::nnz(spy) << "\n";
		rc = grb::FAILED;
	}
	for( const auto &nonzero : spy2 ) {
		if( nonzero.second <= 0 || nonzero.second > 1 ) {
			std::cout << "Invalid entry at spy2( " << nonzero.first.first << ", "
				<< nonzero.first.second << " ): " << nonzero.second
				<< ", expected a value x in the range 0 < x <= 1\n";
			rc = grb::FAILED;
		}
	}
	...
	if( rc == grb::SUCCESS ) {
		...
		rc = rc ? rc :
			grb::eWiseApply( chk, spy, spy2, ring.getMultiplicativeOperator() );
		if( rc == grb::SUCCESS && grb::nnz( chk ) != grb::nnz( spy ) ) {
			std::cerr << "Unexpected number of nonzeroes for chk: " << grb::nnz(chk)
				<< ", expected " << grb::nnz(spy) << "\n";
			rc = grb::FAILED;
		}
		for( const auto &triple : chk ) {
			if( !grb::utils::equals( triple.second, 1.0, 1 ) ) {
				std::cerr << "Verification of normalised spy failed at ( "
					<< triple.first.first << ", " << triple.first.second << " ): "
					<< triple.second << ", expected 1\n";
				rc = grb::FAILED;
			}
		}
	}
	...
	rc = rc ? rc : grb::buildMatrixUnique(
		pattern, reader.cbegin(), reader.cend(), grb::PARALLEL );
	rc = rc ? rc : grb::algorithms::spy( spy2, pattern );
	if( rc == grb::SUCCESS && grb::nnz( spy2 ) != grb::nnz( spy ) ) {
		std::cerr << "Unexpected number of nonzeroes for spy2 (from pattern matrix): "
			<< grb::nnz( spy2 ) << ", expected " << grb::nnz( spy ) << "\n";
		rc = grb::FAILED;
	}
	...
	if( rc == grb::SUCCESS && grb::nnz( spy2 ) != grb::nnz( spy ) ) {
		std::cerr << "Unexpected number of nonzeroes for spy2 (from pattern matrix, "
			<< "normalised): " << grb::nnz( spy2 ) << ", expected " << grb::nnz( spy )
			<< "\n";
		rc = grb::FAILED;
	}
	if( rc == grb::SUCCESS ) {
		...
		rc = rc ? rc : grb::eWiseApply( chk, spy, spy2,
			ring.getMultiplicativeOperator() );
		if( rc == grb::SUCCESS && grb::nnz( chk ) != grb::nnz( spy ) ) {
			std::cerr << "Unexpected number of nonzeroes for chk (pattern): "
				<< grb::nnz(chk) << ", expected " << grb::nnz(spy) << "\n";
			rc = grb::FAILED;
		}
		for( const auto &triple : chk ) {
			if( !grb::utils::equals( triple.second, 1.0, 1 ) ) {
				std::cerr << "Verification of normalised spy (pattern) failed at ( "
					<< triple.first.first << ", " << triple.first.second << " ): "
					<< triple.second << ", expected 1\n";
				rc = grb::FAILED;
			}
	...
	rc = rc ? rc :
		grb::set< grb::descriptors::structural >( boolean, pattern, true );
	rc = rc ? rc : grb::algorithms::spy( spy2, boolean );
	if( rc == grb::SUCCESS && grb::nnz( spy2 ) != grb::nnz( spy ) ) {
		std::cerr << "Unexpected number of nonzeroes for spy2 (from boolean matrix): "
			<< grb::nnz( spy2 ) << ", expected " << grb::nnz( spy ) << "\n";
		rc = grb::FAILED;
	}
	...
	if( rc == grb::SUCCESS && grb::nnz( spy2 ) != grb::nnz( spy ) ) {
		std::cerr << "Unexpected number of nonzeroes for spy2 "
			<< "(from boolean matrix, normalised): " << grb::nnz( spy2 ) << ", expected "
			<< grb::nnz( spy ) << "\n";
		rc = grb::FAILED;
	}
	if( rc == grb::SUCCESS ) {
		...
		rc = rc ? rc :
			grb::eWiseApply( chk, spy, spy2, ring.getMultiplicativeOperator() );
		if( rc == grb::SUCCESS && grb::nnz( chk ) != grb::nnz( spy ) ) {
			std::cerr << "Unexpected number of nonzeroes for chk (boolean): "
				<< grb::nnz(chk) << ", expected " << grb::nnz(spy) << "\n";
			rc = grb::FAILED;
		}
		for( const auto &triple : chk ) {
			if( !grb::utils::equals( triple.second, 1.0, 1 ) ) {
				std::cerr << "Verification of normalised spy (boolean) failed at ( "
					<< triple.first.first << ", " << triple.first.second << " ): "
					<< triple.second << ", expected 1\n";
				rc = grb::FAILED;
			}
