...
void functional_test( const struct test_input & in, struct test_output & out ) {
	...
	const RC rc = grb::dot< grb::descriptors::no_operation >( alpha, xv, yv, reals );
#else
...
#ifdef __clang__
	// Otherwise the reference implementation hits this bug: https://llvm.org/bugs/show_bug.cgi?id=10856)
	const enum RC rc = grb::dot< grb::descriptors::no_operation >( alpha, xv, yv, reals );
#else
...
#ifdef __clang__
		// Otherwise the reference implementation hits this bug: https://llvm.org/bugs/show_bug.cgi?id=10856)
		const enum RC grc = grb::dot< grb::descriptors::no_operation >( alpha, xv, yv, reals );
