...
the location where LPF is installed"
	...
	echo "  --spblas-prefix                     - prefix for spblas routines and for compilation target ('alp_' if none)"
	echo "  --no-solver-lib                     - disable generating the solver libraries"
	echo "  --enable-extra-solver-lib           - enable generating solver library compiled against the reference OMP backend"
