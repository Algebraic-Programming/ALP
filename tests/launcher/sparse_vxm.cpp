...
static grb::RC setupSparseMatrix( grb::Matrix< double > & mx, grb::Vector< double > & c, const size_t n ) {
...
static enum grb::RC checkResult( const grb::Vector< double > & left, const grb::Vector< double > & right ) {
	...
	if( grb::size( left ) != grb::size( right ) ) {
		std::cout << "Left vector does not equal the size of the right "
					 "vector.\n";
		return FAILED;
	}
...
int main( int argc, char ** argv ) {
	...
	if( argc < 3 || argc > 5 ) {
		std::cout << "Usage: " << argv[ 0 ]
				  << " <problem size> <test case> (inner repititions) (outer "
					 "repititions)"
				  << std::endl;
		return 0;
	}
