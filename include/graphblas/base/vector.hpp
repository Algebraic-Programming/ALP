 ...
 *          code sections.
 *
 * \note An implementation may choose to not allocate any data and instead
 *       defer this to Vector::build, Vector::assign, or a free GraphBLAS
 *       function that ends up requiring more capacity than this container
 *       currently has available (e.g., grb::eWiseApply).
 *
 * \note Proper GraphBLAS programs are recommended to follow this
 *       constructor call with a call to Vector::build or Vector::assign to
 *       set the initial capacity and contents. This makes sure subsequent
 *       GraphBLAS calls can achieve their normal performance guarantees.
 */
...
/**
 * Copy constructor. The dimension of this vector will be the same as of
 * the given input vector and cannot change throughout the lifetime of this
 * container. The contents of this container will, however, be independent
 * of the given vector-- the elements from \a x are copied into this
 * container.
 *
 * @param[in] x The GraphBLAS vector to copy.
 *
 * \par Recommended implementation.
 *      This function could call the base constructor Vector() followed by
 *      a call to Vector::assign() using \a x as its sole parameter.
 *
 * \par Performance guarantees
 *      These are inhereted from Vector::Vector() and Vector::assign().
 *
 * \warning Avoid the use of this constructor within performance critical
 *          code sections.
 *
 * @see Vector::assign() for copying \a x without creating a new contianer.
 */
Vector( const Vector< D, implementation, C > & x ) {
	(void)x;
}
