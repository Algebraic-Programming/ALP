...
namespace grb {
	...
	/**
	 * \defgroup reference The nonblocking backend implementation
	 *
	 * Groups all definitions and documentations corresponding to the #nonblocking implementation.
	 * @{
	 */
	...
	namespace config {
		...
		class ANALYTIC_MODEL {
			...
			public:
				...
				/**
				 * The L1 cache size is assumed to be a bit smaller than the actual size to take
				 * into account any data that may be stored in cache and are not considered by
				 * the analytic model, e.g., matrices for the current design.
				 */
				static constexpr double L1_CACHE_USAGE_PERCENTAGE = 0.98;

				/**
				 * It determines whether the tile size is automatically selected by the analytic model or it
				 * is manually selected by the user with the environment variable GRB_NONBLOCKING_TILE_SIZE.
				 */
				static bool manual_tile_size;

				/**
				 * The tile size that is manually selected by the user and is initialized in init.cpp.
				 */
				static size_t manual_fixed_tile_size;

				/**
				 * The maximum number of threads available in the system that may be set with the
		...
		/**
		 * Implementation-dependent configuration parameters for the \a nonblocking backend
		 *
