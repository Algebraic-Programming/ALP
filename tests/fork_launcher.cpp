...
void spmd( lpf_t ctx, lpf_pid_t s, lpf_pid_t P, lpf_args_t args ) {
	...
	(void)printf( "Hello world from PID %d, which has parent %d and local ID "
				  ...
				  "processes.\n",
		getpid(), parent_unix_id, (size_t)s, (size_t)P );
}
...
int main( int argc, char ** argv ) {
	...
	if( MPI_Init( NULL, NULL ) != MPI_SUCCESS ) {
		(void)fprintf( stderr, "MPI_Init returns non-SUCCESS error code.\n" );
		return 10;
	}
	...
	if( sprintf( str, "%d", port ) < 0 ) {
		(void)printf( "Warning: error while transforming the port number %d "
					  "into a string, result is %s.\n",
			port, str );
	}
	...
	if( MPI_Finalize() != MPI_SUCCESS ) {
		(void)fprintf( stderr, "MPI_Finalize returns non-SUCCESS error code.\n" );
		return 20;
	}
		...
		for( size_t i = 1; i < REQ_P; ++i ) {
			...
			do {
				if( waitpid( ids[ i ], &rc, 0 ) == -1 ) {
					(void)fprintf( stderr,
						"Error on call to waitpid. This program may leave ghost "
						"processes, sorry.\n" );
					return 30;
				}
			} while( ! WIFEXITED( rc ) );
			...
			if( rc != 0 ) {
				fail = rc;
				(void)printf( "Child process %zd exited with nonzero exit code "
							  "%d.\n",
					i, (int)rc );
			}
		}
