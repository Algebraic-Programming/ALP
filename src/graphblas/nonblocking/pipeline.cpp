...
Pipeline::Pipeline( const Pipeline &pipeline ) :	stages( pipeline.stages ),
													...
													dense_descr_coordinates( pipeline.dense_descr_coordinates )
{
	...
	// no action is requred regarding the input_output_intersection that is only used temporarily during the pipeline execution
}

Pipeline::Pipeline( Pipeline &&pipeline ) noexcept :	stages( std::move( pipeline.stages ) ),
...
void Pipeline::addStage( const Pipeline::stage_type &&func, const Opcode opcode, const size_t n, const size_t data_type_size, const bool dense_descr, const bool dense_mask,
						void * const output_vector_ptr, void * const output_aux_vector_ptr,
						Coordinates< nonblocking > * const coor_output_ptr, Coordinates< nonblocking > * const coor_output_aux_ptr,
						const void * const input_a_ptr, const void * const input_b_ptr, const void * const input_c_ptr, const void * const input_d_ptr,
						const Coordinates< nonblocking > * const coor_a_ptr, const Coordinates< nonblocking > * const coor_b_ptr,
						const Coordinates< nonblocking > * const coor_c_ptr, const Coordinates< nonblocking > * const coor_d_ptr,
						const void * const input_matrix )
{
	...
	//TODO: does the size of data matches for all containers?

	// pipelines may consist of primitives that operate on different data types, e.g., double and bool
	// the analytic model should take into account the different data types and make a proper estimation
	// an easy and perhaps temporary fix is to use the maximum size of the data types involved in a pipeline
	...
	// special treatment for an SpMV operation as the input must not be overwritten by another stage of the pipeline
	if( opcode == Opcode::BLAS2_VXM_GENERIC ) {
		...
		// TODO once level-3 operations are supported, the following code should be moved
	...
	if( opcode == Opcode::BLAS1_EWISEAPPLY || opcode == Opcode::BLAS1_MASKED_EWISEAPPLY || opcode == Opcode::IO_SET_MASKED_SCALAR || opcode == Opcode::IO_SET_VECTOR || opcode == Opcode::IO_SET_MASKED_VECTOR ) {
		...
		// the second output is always nullptr for the out-of-place primitives that are handled here
		// however, once we have the complete implementation of unzip (which handles sparsity)
		// then I think I need to consider the second output here
	} else {
...
void Pipeline::addeWiseLambdaStage( const Pipeline::stage_type &&func, const Opcode opcode, const size_t n, const size_t data_type_size, const bool dense_descr,
									std::vector< const void * > all_vectors_ptr, const Coordinates< nonblocking > * const coor_a_ptr )
{
	(void) data_type_size;
	...
	// the analytic model takes into account the size of data used by an eWiseLambda primitive
	if( data_type_size > size_of_data_type ) {
		size_of_data_type = data_type_size;
	}
...
{
	// if any of the pipelines contains an out-of-place primitive, the merged pipeline contains as well
	if( pipeline.contains_out_of_place_primitive ) {
		contains_out_of_place_primitive = true;
	}

	// the size of the data accessed in the pipeline is updated based on the maximum of the two merged pipelines
...
void Pipeline::clear()
{
	// after executing the pipeline, the size of vectors should be reset to indicate an empty pipeline
	contains_out_of_place_primitive = false;
	containers_size = 0;
	size_of_data_type = 0;
...
void Pipeline::buildAlreadyDenseVectors()
{
	...
	// the intersection of the sets "dense_descr_coordinates" and "accessed_coordinates" is usually empty except for the output of
	// an out-of-place operation that is always added in the set of "accessed_coordinates" regardless the dense descriptor
...
grb::RC Pipeline::verifyDenseDescriptor()
{
	...
	// the coordinates for all vectors that are accessed in a primitive with the dense descriptor should be dense after the execution of the pipeline
	// otherwise, the dense descriptor was used illegally for vectors that were not dense
	// for all primitives with the dense descriptor, the coordinates of all vectors are added in the set "dense_descr_coordinates"
	// for which the local coordinates are not buit, not accessed, and not updated
	// therefore, a sparse vector can become dense only if the same vector is used in different primitives, i.e., with and without the dense descriptor
	// this is a case that the dense descriptor may be used illegally, but the following code cannot catch it
	for( std::set< Coordinates<nonblocking > * >::iterator it = dense_descr_coordinates.begin(); it != dense_descr_coordinates.end(); ++it ) {
		if( !( *it )->isDense() ) {
			return ILLEGAL;
		}
...
grb::RC Pipeline::execution()
{
	...
	// all operations stored in the pipeline are cleared and the function returns immediately
	if( containers_size == 0 ) {
		clear();
		return ret;
	}

	// compute the intersection of the input and output vectors that should be subtracted from the number of accessed vectors
	...
	// if all vectors are already dense and there is no out-of-place operation to make them sparse
	// we avoid paying the overhead for updating the coordinates for the output vectors
	if( all_already_dense_vectors && !contains_out_of_place_primitive ) {
		...
		for( size_t tile_id = 0; tile_id < num_tiles; ++tile_id ) {
			...
			for( std::set< internal::Coordinates< nonblocking > * >::iterator vt = vbegin(); vt != vend(); ++vt ) {
				...
				// skip the initialization of coordinates of different size, which may happen only for
				// the input of vxm_generic as it's read-only for the current design
		...
		// even if only one vector is sparse, we cannot reuse memory because the first two arguments that we pass to
		// the lambda functions determine whether we reuse memory or not and they cannot vary for different vectors
		#pragma omp parallel for schedule(dynamic) num_threads(nthreads)
		for( size_t tile_id = 0; tile_id < num_tiles; ++tile_id ) {
		...
		if( initialized_coordinates ) {
			...
			// compute the prefix sums for each vector and store them in the last part of _buffer
			// the computation for different vectors may run in parallel but is not preferred to avoid high overhead
			for( std::set< internal::Coordinates< nonblocking > * >::iterator vt = vbegin(); vt != vend(); ++vt ) {
...
#ifdef _NONBLOCKING_DEBUG
	if( ret == ILLEGAL ) {
		std::cerr << "error in pipeline execution: the dense descriptor was illegally used" << std::endl;
	}
#endif
